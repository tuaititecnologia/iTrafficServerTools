# Set all databases to SIMPLE recovery mode
...
function Set-DatabaseRecoverySimple {
    param(
        [string]$ServerInstance,
        [string]$DatabaseName
    )
    ...
    $query = @"
SET NOCOUNT ON;
ALTER DATABASE [$escapedName] SET RECOVERY SIMPLE;
"@
...
try {
    ...
    Write-Host ""
    $confirm = Read-Host "Do you want to change ALL databases to SIMPLE recovery mode? (Y/N)"
    
    if ($confirm -eq 'Y' -or $confirm -eq 'y') {
        Write-Host "`nChanging recovery mode to SIMPLE..." -ForegroundColor Yellow
        ...
        foreach ($db in $databases) {
            ...
            $isSimple = $false
            if ($currentModel) {
                $isSimple = ($currentModel.ToUpperInvariant() -eq 'SIMPLE')
            }

            if (-not $isSimple) {
                try {
                    Set-DatabaseRecoverySimple -ServerInstance $serverInstance -DatabaseName $db.Name
                    $db.RecoveryModel = 'SIMPLE'
                    Write-Host "  Changed: $($db.Name) -> SIMPLE" -ForegroundColor Green
                    $changedCount++
                } catch {
                    Write-Host "  Error changing $($db.Name): $($_.Exception.Message)" -ForegroundColor Red
                }
            } else {
                Write-Host "  Skipped: $($db.Name) (already SIMPLE)" -ForegroundColor Gray
                $skippedCount++
            }
        }
